...
def real_queue(synchronous_queue):
    """Override synchronous_queue and run a real CallbackQueue worker for this test.

    synchronous_queue is listed as a dependency so it sets up first; we then
    replace its patch with the real add method and start a proper worker thread.
    Each test gets a fresh worker; teardown calls stop() to drain and halt it.
    """
