...
class TestRenderInstructions:
    ...
    def test_every_instruction_renders_one_frame(
        ...
        rendered = render_instructions(pulse_stream, ChannelName.PULSE1, config)
    ...
    def test_a_channel_describing_no_frame_raises(self, config: Config) -> None:
        with pytest.raises(ValueError):
            render_instructions([], ChannelName.PULSE1, config)
...
class TestRenderGenerators:
    ...
    @pytest.fixture
    def streams(self, pulse_stream: List[InstructionUnion]) -> Dict[ChannelName, List[InstructionUnion]]:
        return {
            ChannelName.PULSE1: pulse_stream,
            ChannelName.TRIANGLE: [TriangleInstruction(on=True, pitch=48)],
            ChannelName.NOISE: [NoiseInstruction(on=True, period=4, volume=15, short=False)],
        }

    def test_every_sounding_channel_renders(
        self,
        streams: Dict[ChannelName, List[InstructionUnion]],
        config: Config,
    ) -> None:
        assert set(render_channels(streams, config)) == set(streams)
    ...
    def test_a_channel_standing_by_renders_nothing(
        ...
        streams[ChannelName.PULSE2] = []
        assert ChannelName.PULSE2 not in render_channels(streams, config)
    ...
    def test_the_channels_come_back_in_channel_order(
        ...
        rendered = render_channels(streams, config)
        assert list(rendered) == [name for name in ChannelName.items() if name in streams]
    ...
    def test_a_channel_sounds_what_it_renders_on_its_own(
        ...
        rendered = render_channels(streams, config)
        for channel_name, instructions in streams.items():
            assert np.array_equal(
                rendered[channel_name],
                render_instructions(instructions, channel_name, config),
            )
