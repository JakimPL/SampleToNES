...
class TestWhereAStreamIsReEntered:
    ...
    def test_the_first_tick_is_the_streams_own_first_byte(self) -> None:
        ...
        assert stream_entry(stream, FIRST_TICK, DICTIONARY) == 0
    ...
    def test_a_tick_further_in_walks_past_every_token_before_it(self) -> None:
        tokens = [
            ...
            HoldToken(ticks=3),
            PhraseToken(phrase_id=PHRASE_ID, ticks=4, transpose=SHIFT, default=False),
        ]
        stream = emit(tokens)
        covered = sum(token.ticks for token in tokens[:2])
        assert stream_entry(stream, covered, DICTIONARY) == sum(token.size for token in tokens[:2])
    ...
    def test_a_tick_a_token_spans_is_refused(self) -> None:
        ...
        with pytest.raises(ValueError, match="spans position"):
            stream_entry(stream, 3, DICTIONARY)
