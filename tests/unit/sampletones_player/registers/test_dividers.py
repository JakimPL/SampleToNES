...
class TestAnchoredPitches:
    ...
    def test_a_bend_past_halfway_is_counted_from_the_frames_own_note(self) -> None:
    ...
    def test_a_bend_past_the_byte_is_counted_from_the_nearest_pitch(self) -> None:
        divider = PLAYER_TIMER_TABLE[PLAYER_REFERENCE_PITCH] - SIGNED_BYTE_LIMIT - 1
        assert anchored_pitches((PLAYER_REFERENCE_PITCH,), (divider,), PLAYER_TIMER_TABLE) == (
            nearest_pitch(PLAYER_TIMER_TABLE, divider).pitch,
        )

    def test_notes_and_dividers_covering_different_frames_are_refused(self) -> None:
        with pytest.raises(ValueError):
