...
INFORMATION: Final[NSFInformation] = NSFInformation(
    title="Amen",
    artist="Author",
)

SOUNDING: Final = pulse_tick(
    PLAYER_FULL_VOLUME,
    0,
    PLAYER_REFERENCE_TIMER,
)
RESTING: Final = pulse_tick(
    PLAYER_SILENT_VOLUME,
    0,
    PLAYER_REFERENCE_TIMER,
)
...
def song() -> Song:
    return player_song(
        resting_streams((SOUNDING, RESTING)),
        NTSC_FREQUENCY,
        loop_tick=None,
    )
...
class TestNSFBytes:
    ...
    def test_the_file_leads_with_its_header(
        self,
        song: Song,
        image: DriverImage,
    ) -> None:
        assert nsf_to_bytes(song, INFORMATION, image)[:HEADER_SIZE] == header_to_bytes(
            INFORMATION,
            image.addresses,
        )

    def test_the_driver_follows_the_header(
        self,
        song: Song,
        image: DriverImage,
    ) -> None:
    ...
    def test_the_song_follows_the_driver(
        self,
        song: Song,
        image: DriverImage,
    ) -> None:
        data = nsf_to_bytes(song, INFORMATION, image)
        assert data[HEADER_SIZE + len(image.code) :] == song_to_bytes(
            song,
            PROGRAM_SIZE - len(image.code),
        )

    def test_the_file_is_its_three_parts_and_nothing_more(
        self,
        song: Song,
        image: DriverImage,
    ) -> None:
    ...
    def test_the_loaded_image_fits_the_program_area(
        self,
        song: Song,
        image: DriverImage,
    ) -> None:
...
class TestSongsBeyondTheProgramArea:
    ...
    def test_a_song_too_large_for_the_program_area_raises(
        self,
        image: DriverImage,
    ) -> None:
        with pytest.raises(SongTooLargeError):
            nsf_to_bytes(
                oversized_song(image),
                INFORMATION,
                image,
            )
...
class TestWriteNSF:
    ...
    def test_the_file_holds_the_bytes_the_song_serializes_to(
        ...
        assert destination.read_bytes() == nsf_to_bytes(
            song,
            INFORMATION,
            image,
        )
