...
SUBFOLDER: Final[str] = "takes"

SUBFOLDER_THE_READER_OPENED: Final[str] = with_rows_open(STARRED_CONFIGURATION, SUBFOLDER)
THE_WAY_DOWN_TO_THE_READERS_ROW: Final[str] = with_rows_open(
    WHOLE_TREE,
    BY_CONFIGURATION,
    FREQUENCIES,
    TRANSFORMATION,
...
WHOLE_TREE_WITHOUT_THE_ARCHIVE: Final[str] = without_branch(WHOLE_TREE, ARCHIVE)
...
class TestTheReadersShape:
    ...
    def test_a_row_the_reader_opened_is_drawn_open(self, corpus: BrowserCorpus) -> None:
        panel = build_browser_panel(corpus, paths_of(corpus, "C"), favorites_only=True)
        assert render_view(panel) == STARRED_CONFIGURATION

        set_row_expanded(panel, row_named(corpus, SUBFOLDER), expanded=True)
    ...
    def test_a_row_the_reader_closed_is_drawn_closed(self, corpus: BrowserCorpus) -> None:
        ...
        set_row_expanded(panel, row_named(corpus, SUBFOLDER), expanded=False)
    ...
    def test_the_way_down_to_a_row_the_reader_opened_stands_once_the_mode_goes_off(
        ...
        for label in (BY_CONFIGURATION, FREQUENCIES, TRANSFORMATION, CONFIGURATION_B_ROW):
            set_row_expanded(panel, row_named(corpus, label), expanded=True)

        set_row_expanded(panel, row_named(corpus, TRANSFORMATION), expanded=False)
        select_favorites(panel)
        render_view(panel)
        deselect_favorites(panel)
    ...
    def test_the_way_down_the_mode_hands_over_is_written_down_with_the_readers_rows(
        ...
        heading = row_named(corpus, TRANSFORMATION)
        set_row_expanded(panel, row_named(corpus, CONFIGURATION_B_ROW), expanded=True)
        select_favorites(panel)
        render_view(panel)
    ...
    def test_a_row_the_mode_never_drew_keeps_the_state_it_had(self, corpus: BrowserCorpus) -> None:
        panel = build_browser_panel(corpus, paths_of(corpus, "A/beat"), favorites_only=False)
