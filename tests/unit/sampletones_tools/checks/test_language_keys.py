...
from sampletones_tools.checks import language_keys as check_language_keys
from sampletones_tools.checks.source.lookups import LookupSite
from sampletones_tools.checks.source.modules import source_paths
from sampletones_tools.checks.source.values import EnumTable
...
class TestMain:
    def test_the_repository_and_its_language_file_agree(self) -> None:
        assert dispatch(COMMANDS, ["check", "language-keys"]) == 0
    ...
    def test_a_disagreement_is_reported_by_kind(
        ...
        exit_code = dispatch(
            COMMANDS, ["check", "language-keys", "--source", str(source), "--language-file", str(entries)]
        )
