...
from sampletones_tools.checks import tag_names as check_tag_names
from sampletones_tools.checks.source.modules import SourceModule, source_paths
...
class TestMain:
    def test_the_tags_package_names_its_tags_after_them(self) -> None:
        assert dispatch(COMMANDS, ["check", "tag-names", "--all"]) == 0
    ...
    def test_a_mistyped_tag_is_reported_where_it_sits(
        ...
        assert dispatch(COMMANDS, ["check", "tag-names", str(module)]) == 1
    ...
    def test_a_well_named_module_reports_nothing(
        ...
        assert dispatch(COMMANDS, ["check", "tag-names", str(module)]) == 0
