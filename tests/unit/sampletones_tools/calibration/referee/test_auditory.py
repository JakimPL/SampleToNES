...
from sampletones_tools.calibration.referee.auditory import MultiResolutionAuditoryReferee
...
class TestMultiResolutionAuditoryReferee:
    def test_identical_signals_score_zero(self, referee: MultiResolutionAuditoryReferee) -> None:
        tone = _tone(440.0)
        assert referee.judge(tone, tone).score == pytest.approx(0.0, abs=1e-9)
    ...
    def test_score_grows_with_the_distortion_level(self, referee: MultiResolutionAuditoryReferee) -> None:
        ...
        slightly_noisy = referee.judge(tone, tone + 0.01 * noise).score
        very_noisy = referee.judge(tone, tone + 0.2 * noise).score
    ...
    def test_related_timbre_scores_closer_than_unrelated_noise(
        ...
        assert referee.judge(target, square).score < referee.judge(target, noise).score
    ...
    def test_score_is_invariant_under_a_common_gain(self, referee: MultiResolutionAuditoryReferee) -> None:
        ...
        base = referee.judge(tone, estimate).score
        scaled = referee.judge(0.25 * tone, 0.25 * estimate).score
