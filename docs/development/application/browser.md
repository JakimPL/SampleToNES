...
5. **One thing may stand in several places.** A reconstruction is listed by the configuration that produced it and again by the audio it was made from. An action on the thing, and not on the row, asks for every row representing it (`Tree.find_nodes`, `BrowserManager.nodes_at`) and hands them to both tabs.
