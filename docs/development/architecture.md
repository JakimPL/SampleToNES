...
- Structural depth themes have exactly one owner each: the `TabColumns` scaffold binds the recessed GROUND theme to a column, and the `card()` context manager binds the raised SURFACE theme to a card. Panels and coordinators bind only semantic/content themes (a per-generator checkbox tint, the player toolbar), never GROUND or SURFACE.
