...
| Package | Purpose |
|---------|---------|
| `config/` | `ConfigManager` (domain generation config), `SessionManager` (runtime session: last paths, audio device, window geometry). A session file outlives the files and folders it names, so every path it holds is read against the disk as it loads: an open file that is gone is forgotten, a remembered folder moves to the nearest one standing above it, and a favorite that is gone leaves the list. Presentation-free: it records load outcomes (`ConfigLoadOutcome`) as domain data for `ConfigCoordinator` to present. Must not import the visual packages, `coordinators/`, or `application.py` |
