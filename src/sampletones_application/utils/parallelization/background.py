...
def stop_background_workers(timeout: float = SHUTDOWN_JOIN_TIMEOUT) -> None:
    ...
    cancellation point instead of the join waiting out its natural duration.
    Stopping the callback queue then discards any results still pending delivery
    to the main thread, and the concurrent executor threads touch the dearpygui
    context, so both are halted and awaited before that context is destroyed.
    """
