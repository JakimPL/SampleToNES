...
class SelectionReader(Protocol):
    ...
    An application answering nothing in that span leaves the text unknown, which reads as
    ``None``.
    """
...
class X11TextClipboard:
    ...
    answers nothing at all. The frames keep being drawn meanwhile, which is also what lets this
    ...
    def __init__(self, reader: SelectionReader) -> None:
        ...
        self._waiting: List[ClipboardTextCallback] = []
    ...
    def _transfer(self) -> None:
        """Reads on the worker and hands the answer to the render thread, none where the read fails.
    ...
    def _answer(self, text: Optional[str]) -> None:
