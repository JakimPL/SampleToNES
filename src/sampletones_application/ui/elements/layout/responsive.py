...
def expanded_side_width(
    ...
    viewport wider than ``baseline_viewport_width`` offers is shared out with the centre taking twice
    the share of each side. Splitting the surplus ``2 + side_panel_count`` ways and granting one share
    to each side keeps the centre the widest column while the sides breathe on large displays. At the
    baseline the column sits at its configured ``base_width`` and grows only as surplus appears above it.
    """
