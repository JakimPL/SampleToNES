...
class GUINSFExportWindow(GUISeededDialogWindow[NSFExportViewModel]):
    ...
    def _create_destination(self) -> None:
        ...
        with labeled_field(
            ...
            dpg.add_group(
                horizontal=True,
                tag=TAG_SETTINGS_NSF_GROUP_DESTINATION,
            )
    ...
    def _render(self) -> None:
        ...
        dpg_configure_item(
            TAG_SETTINGS_NSF_BUTTON_EXPORT,
            enabled=view_model.export_enabled,
        )
    ...
    def _render_channels(self, view_model: NSFExportViewModel) -> None:
        ...
        dpg_configure_item(
            TAG_SETTINGS_NSF_TEXT_NO_CHANNEL,
            show=not view_model.export_enabled,
        )
    ...
    def _render_repeat(self, view_model: NSFExportViewModel) -> None:
        ...
        dpg_configure_item(
            TAG_SETTINGS_NSF_COMBO_REPEAT,
            items=list(self._repeats_by_label),
        )
        dpg_set_value(
            TAG_SETTINGS_NSF_COMBO_REPEAT,
            self._repeat_labels[view_model.choices.repeat],
        )
        dpg_configure_item(
            TAG_SETTINGS_NSF_GROUP_LOOP_FRAME,
            show=view_model.loop_frame_visible,
        )
        dpg_configure_item(
            TAG_SETTINGS_NSF_INPUT_LOOP_FRAME,
            enabled=view_model.loop_frame_visible,
        )
        self._settle_input(
            TAG_SETTINGS_NSF_INPUT_LOOP_FRAME,
            view_model.loop_frame_label(self._fmt_frame),
        )
        dpg_set_value(
            TAG_SETTINGS_NSF_TEXT_FRAME_COUNT,
            view_model.frame_count_label(self._fmt_frame_count),
        )
        dpg_set_value(
            TAG_SETTINGS_NSF_TEXT_LENGTH,
            view_model.length_label(self._fmt_length),
        )
    ...
    def _render_scheme(self, view_model: NSFExportViewModel) -> None:
        ...
        dpg_configure_item(
            TAG_SETTINGS_NSF_COMBO_SCHEME,
            items=list(self._schemes_by_label),
        )
        dpg_set_value(
            TAG_SETTINGS_NSF_COMBO_SCHEME,
            self._scheme_labels[scheme],
        )
        dpg_set_value(
            TAG_SETTINGS_NSF_TEXT_SCHEME_DESCRIPTION,
            self._scheme_descriptions[scheme],
        )
    ...
    def _on_scheme_changed(self, _sender: Sender, app_data: str) -> None:
        view_model = self.view_model
        self._emit(
            view_model.choices.with_scheme(self._schemes_by_label[app_data], view_model.offer),
        )
