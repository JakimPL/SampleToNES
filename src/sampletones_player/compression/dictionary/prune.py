...
def prune(
    ...
    phrases named most often take them and the tokens naming those shed a byte each. Each phrase
    kept carries the count its own tokens play it at most often, which is the count a token may
    then leave unstated.
    ...
    Args:
        ...
        counts: The count each phrase's tokens play it at most often.
    ...
    return PhraseTable(
        phrases=tuple(table[entry.phrase_id].model_copy(update={"default": counts[entry.phrase_id]}) for entry in kept)
    )
