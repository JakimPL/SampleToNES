...
def check_cases(roots: Sequence[Path]) -> List[Finding]:
    """Every case under the roots comparing text it rendered with a literal it spelled out."""
    return findings(discover_modules(roots))


def report(found: Sequence[Finding]) -> int:
