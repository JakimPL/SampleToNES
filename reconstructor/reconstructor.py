...
class Reconstructor:
    def __init__(self, config: Config) -> None:
        ...
        self.generators = {name: generator(config) for name, generator in GENERATORS.items()}
    ...
    def __call__(self, audio: np.ndarray) -> Reconstruction:
        ...
        for fragment_id in tqdm(range(len(fragments))):
            ...
            partial_reconstruction, partial_instructions, error = self.find_best_fragment_approximation(self.state)

            self.state.approximations.append(partial_reconstruction)
            for name, instruction in partial_instructions.items():
                instructions[name].append(instruction)

            total_error += error
            self.update_phases()

        approximation = np.concatenate(self.state.approximations)
        return Reconstruction.from_results(approximation, instructions, total_error)
    ...
    def find_best_fragment_approximation(
        ...
        return approximation, instructions, total_error
