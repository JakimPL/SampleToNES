...
def find_violations(filepath: Path) -> list[tuple[str, str]]:
    ...
    for line_number, line in enumerate(filepath.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if DEARPYGUI_IMPORT_RE.match(line):
            violations.append(("dearpygui import", f"{location}: {line.strip()}"))
        elif UI_IMPORT_RE.match(line):
            violations.append(("ui layer import", f"{location}: {line.strip()}"))
    return violations
...
def main() -> None:
    ...
    if all_violations:
        print("boundary violation(s) found in logic/view_model:", file=sys.stderr)
